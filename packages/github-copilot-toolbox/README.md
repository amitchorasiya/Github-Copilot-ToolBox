...
Screenshots live in the monorepo [`screenshots/`](https://github.com/amitchorasiya/Github-Copilot-ToolBox/tree/main/screenshots) folder (repo root). Embeds use **`https://github.com/amitchorasiya/Github-Copilot-ToolBox/raw/main/screenshots/…`** so images work on **GitHub** and in the **VS Code** extension details view. The published `.vsix` README is generated from the root README with URLs rewritten to **`raw.githubusercontent.com`** by the packaging script.
